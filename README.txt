...
1) This README.txt, which contains:
	a) This overview of files in the project folder.
	b) A comparison between my replicated results and the results of the paper.
	c) A discussion of my MultiTree results.
	d) A future research suggestion.
2) The original paper this project was based on. This is the file ThePaper.pdf.
3) My main results. Namely, replicating the results of the paper.
	a) My implementation can be found in the file main.hs. 
...
4) My first additional assignment in which I implemented a Church and CoChurch encoding for MultiTrees.
   MultiTrees here are trees with any number of children, and values stored at the leafs.
	a) The implementation can be found in the file multitree.hs.
	   ...
	   Otherwise, the results will not be duplicated as the defined Haskell optimization rules will not be used.
	b) My results can be found in the file ResultsMultiTree.pdf.
	c) I discuss these results at the end of this README file.
5) My second additional assignment in which I implemented a Church encoding for lists.
	a) The implementation can be found in the file list.hs.
	...
	b) An explanation of this implementation can be found in the file ListChurchEncodingExplanation.pdf
...
Finally, I propose one potentially interesting future research idea:
...
If anything is unclear (for example regarding the compilation of the files), please feel free to contact me on MatterMost or email (m.f.bertorotta@student.tudelft.nl).
-Matteo Bertorotta